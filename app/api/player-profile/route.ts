...
const ROBLOX = 'https://apis.roblox.com';
// Use direct roblox.com subdomains – Cloudflare Workers can reach these fine
const API = {
...
// Safely fetch JSON – never throws, returns null on any failure
...
export async function GET(request: Request) {
  ...
  // ── Step 1: Critical – fetch user info first ──────────────────────────────
  const userData = await safeJson(`${API.users}/v1/users/${userId}`);

  if (!userData) {
    return NextResponse.json(
      { error: 'Failed to reach Roblox API. Please try again.' },
      { status: 503 },
    );
  }
  ...
  // ── Step 3: Avatar image & outfit items ─────────────────────────────────
  ...
  if (friends.length > 0) {
    ...
    (usersData?.data ?? []).forEach((u: any) => userInfoMap.set(String(u.id), u));
  ...
  return NextResponse.json({
    ...
    games,
    usernameHistory: (usernameHistoryData?.data ?? []).map((h: any) => h.name),
  });
}
